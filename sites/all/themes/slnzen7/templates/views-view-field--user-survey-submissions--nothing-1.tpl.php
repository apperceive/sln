 ...
 */
  ...
  $surveynode = node_load($nid);
  
  // TODO: what if !$surveynode
  
  // TODO: check that submission status is not draft
  
  
  $survey_subject_tid = $surveynode->field_sln_survey_subject[LANGUAGE_NONE][0]['tid'];
  if (($survey_subject_tid == SLN_SUBJECT_SELF) && !$submission->is_draft) {
    $output = l('report', "slnsurvey/submission/report/$nid/$sid");
    $output .= "&nbsp;";
    $output .= "<a title='Download or view PDF version of report...' target='_blank' class='sln-download-pdf' href='/printpdf/slnsurvey/submission/report/$nid/$sid?pdf=yes'><span>PDF</span></a>";
  }
  else {
    $output = "";
  } // if 
